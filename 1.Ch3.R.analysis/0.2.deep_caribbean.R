...
write.csv(carib.deep.fishbase,"file.fish.carib.grouped.csv")
...
#### 1. abundance and biomass by depths and location ####
carib.deep.traits <- read.csv(file = "carib.deep.traits.csv") %>%
  select(-X)
...
#### 2. Biomass and species richness, by depth and location ####
## biomass
...
#### 3.1. Size structure and community composition - PCA ####
carib.deep.traits <- read.csv(file = "carib.deep.traits.csv") %>%
  select(-X)
carib.deep.fishbase <- read.csv("file.fish.carib.grouped.csv")
...
## and then returning a subset of those points using the indices computed by chull. The result is a selection of points from the original data that form the convex hull.
carib.deep.hull <- 
  carib.deep.points %>% 
...
#### 3.2. Size structure and community composition - dendogram ####
...
## Similarity Profile Analysis (SIMPROF) test to assess the statistical significance 
## of clusters in multivariate data. 
## clustsig package not on CRAN anymore (function simprof)
...
ggsave(carib.dend.plot, file =  "Figures/carib.dend.plot.png", width = 10, height = 8)

#### 3.3. Community structure:PCA and SIMPROF by location ####
#### 3.3.1. Curacao ####
...
for (i in 1:length(cur.simsum)){
  spec <- as.data.frame(cur.simsum[i]) %>%
    ...
    select(1,8) %>% # i think mistake was from here, "1,7" instead of "1,8"
    filter(.[[1]] > 0.05) %>% 
    #select(1,7) %>% 
    # filter(. > 0.05) %>% ## error here, because evaluates 2 logical test (2 columns), only 1 can be done
    select(species)
...
#### 3.3.2. Bonaire (pb w clustsig package) ####
...
#### 3.3.3. Roatan (pb w clustsig package) ####
...
#### 3.3.4. St. Eustatius (pb w clustsig package) ####
...
#write.csv(sf.com.cl,"file_sf.com.cl.csv")
...
#### 4. trait based analysis: functional richness, evenness and Rao's q #####
#### 4.0: data prep ####
carib.deep.traits <- read.csv(file = "carib.deep.traits.csv") %>%
  select(-X)
...
t.trophic <- read.csv("traits_trophic.csv") %>%
  ...
  filter(sum > 0) %>% # remove species that are not assigned any trophic levels
  select(-sum) %>%
...
t.diet <- read.csv("traits_diet.csv") %>%
  ...
  rename(species.traits = 1) %>%
  ## grouping some diet categories together
...
## the more categories within a trait, the lower the weight.
## this is because for qualitative traits, the matrix of traits will feature each 
## category of trait (e.g., planktivore, carnivore, herbivore) as its own column (as if it was its own trait).
## Because what we really want to give equal weight to are functional traits, and not each trait categories, 
## we assign a fraction of the weight to each trait category. 
...
#### 4.1. FD Curacao (across depth) ####
cur.com.cl <- read_csv("file_Curacao_community_cluster.csv")
cur.com.cl <- cur.com.cl[,-1] #deleting depth range column
...
# cur.traits.rn: binary matrix (0,1) for each trait category and each species 
# cur.com.fd: matrix of relative abundance of species 
# corr: correction method to use when the species-by-species distance matrix cannot be represented in a Euclidean space
...
fd.cur
cur.fd.res <- data.frame("dband" = as.numeric(rownames(cur.com.fd))*10+30, #getting depth range back
                         "location" = "Curacao",
                         "spric" = fd.cur$nbsp,
                         "fric" = fd.cur$FRic,
                         "feve" = fd.cur$FEve,
...
#### 4.2 FD Bonaire (across depth) ####
bon.com.cl <- read.csv("file_bon_com_cl.csv", check.names=FALSE)
bon.com.cl <- bon.com.cl[,-1] #deleting depth range column
...
fd.bon <- dbFD(bon.traits.rn, bon.com.fd, w = trait.weights, w.abun = TRUE, 
               CWM.type = "all", m = 2, corr = "cailliez", stand.FRic = TRUE)
fd.bon

bon.fd.res <- data.frame("dband" = as.numeric(rownames(bon.com.fd))*10+30,
                         "location" = "Bonaire",
                         "spric" = fd.bon$nbsp,
                         "fric" = fd.bon$FRic,
                         "feve" = fd.bon$FEve,
...
#### 4.3. FD Roatan (across depth) ####
roa.com.cl <- read.csv("file_roa.com.cl.csv", check.names=FALSE) 
roa.com.cl <- roa.com.cl[,-1] #deleting depth range column
...
roa.fd.res <- data.frame("dband" = as.numeric(rownames(roa.com.fd))*10+30,
                         "location" = "Roatan",
                         "spric" = fd.roa$nbsp,
                         "fric" = fd.roa$FRic,
                         "feve" = fd.roa$FEve,
...
#### 4.4. FD Statia (across depth) ####
sta.com.cl <- read.csv("file_sta.com.cl.csv", check.names=FALSE) #relative abundance by depth (one line per depth band)
sta.com.cl <- sta.com.cl[,-1] #deleting depth range column
...
sta.fd.res <- data.frame("dband" = as.numeric(rownames(sta.com.fd))*10+30,
                         "location" = "Statia",
                         "spric" = fd.sta$nbsp,
                         "fric" = fd.sta$FRic,
                         "feve" = fd.sta$FEve,
...
fd.res.all <- bind_rows(cur.fd.res, bon.fd.res, roa.fd.res, sta.fd.res) %>%
  drop_na(fric) %>% ## removes lines with NA. 
  mutate(location = as.factor(location))

#write.csv(fd.res.all,"file_FD.all.islands.csv")
GGally::ggpairs(fd.res.all)


#### 4.5. Functional diversity graphs ####
FD.values <- read.csv("file_FD.all.islands.csv")

### functional diversity 
ggplot(data=FD.values, aes(x=dband,y=spric,color=location))+
  geom_point()+
  theme_classic()

#### 5.1. GAMs: species richness ~ location + depth - NO BUENO ####
## pb here is that relies on data from functional diversity analysis which has been modified to
# drop all lines with NAs for functional diversity, which leads to deleting all lines for which
# spric <3

## gam: generalized additive models. GAMs are a type of regression model that extends traditional linear models 
# by allowing for nonlinear relationships between predictors and the response variable. 
fd.res.all <- read.csv("file_FD.all.islands.csv")
spric.gam = gam(spric ~ s(dband) + location, family = poisson, data = fd.res.all)
summary(spric.gam)
# location has no influence on species richness, but depth has. 

# empty table with 1 line per (meter depth x site) to fill with predicted species richness
...
# makes a prediction based on all the data pooled across sites, and 
# also applies specific coefficient for each site value given results from gam model
...
#### 5.2. Raos'sQ GAM ~ location + depth ####
rao.gam <- gam(raoq ~ s(dband) + location, data = fd.res.all)
summary(rao.gam)
# location and depth have influence on Rao Q. 
